...
st.title("⚔️ 광역 패링 액션 (보스 돌진 하향 및 파란색 큰적 패턴 추가, 멈춤 현상 원인 수정)")
st.caption("WASD: 이동 | Spacebar: 패링 | Shift: 불공격 | 파란색 큰적(탱커) 전용 공격 패턴 부여")
...
<body>
    ...
    <script>
        ...
        function spawnEnemy() {
            ...
            let isTank = randType >= 0.35 && randType < 0.55; // 파란색 큰적 (탱커)
        ...
        function update() {
            ...
            enemies.forEach((enemy) => {
                ...
                if (enemy.type === "melee") {
                    ...
                    // 🛡️ 파란색 큰 적(탱커): 강력한 근접 강타 및 충격파 패턴 추가
                    let attackRange = 110;
                    if (enemy.state === "chase") {
                        enemy.x += (dx / dist) * enemy.speed; enemy.y += (dy / dist) * enemy.speed;
                        if (dist < attackRange) { enemy.state = "tank_windup"; enemy.stateTimer = 0; }
                    } else if (enemy.state === "tank_windup") {
                        // 공격 전조 (멈춰서 준비)
                        if (enemy.stateTimer > 50) {
                            enemy.state = "tank_slam"; enemy.stateTimer = 0;
                        }
                    } else if (enemy.state === "tank_slam") {
                        // 바닥 내려찍기 충격파 발동
                        screenShake = 15;
                        effectRings.push({ x: enemy.x, y: enemy.y, radius: enemy.radius, maxRadius: 130, life: 15, maxLife: 15, color: "59, 130, 246" });
                        
                        if (player.isParrying && dist < 140) {
                            executeParry(enemy.x, enemy.y, false);
                        } else if (dist < 130 && player.invincibleTimer <= 0) {
                            let hitDmg = 25;
                            player.hp -= hitDmg;
                            addDamageText(player.x, player.y - 20, `-${hitDmg} HP`, "#ff0000");
                        }
                        ...
                        if (enemy.stateTimer > 45) { enemy.state = "chase"; enemy.stateTimer = 0; }
                    }
                        ...
                        if (enemy.stateTimer > 45) {
                            ...
                            if (dist < 150) { 
                                if (player.isParrying) {
                                    ...
                                    let bossSlamDmg = 18; // 보스 돌진/내려찍기 대미지 하향
                                    player.hp -= bossSlamDmg;
                                    addDamageText(player.x, player.y - 20, `-${bossSlamDmg} HP`, "#ff0000");
                                }
                            }
            ...
            for (let i = projectiles.length - 1; i >= 0; i--) {
                ...
                if (pDist < player.radius + p.radius + 25) {
                    if (player.isParrying) {
                        ...
                        projectiles.splice(i, 1); // 패링 시 투사체 안전하게 즉시 제거 (메모리 릭 및 멈춤 방지)
        ...
        function draw() {
            ...
            enemies.forEach(enemy => {
                ...
                if (enemy.type === "tank") {
                    ctx.fillStyle = enemy.state === "tank_windup" ? "#93c5fd" : "#3b82f6";
                } else if (enemy.type === "ranged") {
                ...
                if (enemy.type === "tank" && enemy.state === "tank_windup") {
                    ctx.fillStyle = "#60a5fa";
                    ctx.font = "bold 12px sans-serif";
                    ctx.textAlign = "center";
                    ctx.fillText("🛡️ 강타 준비!", enemy.x, enemy.y - enemy.radius - 12);
                } else if (enemy.type === "ranged" && enemy.state === "windup") {
